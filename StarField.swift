...
class StarField: SKNode {
    ...
    func initSetup() {
        ...
        // Because we need to call a method on self (launchStar) from
        // inside a code block {}'s, we must create a weak reference to self.
        // This is what we will be doing below with [weak self] and then
        // eventually assigning the "weak" self to a local constant called
        // weakSelf.
        //
        // Why?
        //
        // The run action holds a strong reference to the code block and
        // the node (self) holds a strong reference to the run action.  If
        // the code block held a strong reference to the node (self), then
        // the run action, the code block, and the node (self) would all
        // hold strong references to each other forming a retain cycle which
        ...
        // Run our action after a short delay
    ...
    func launchStar() {
        ...
        if let scene = self.scene {
            ...
            // Move the star toward the bottom of the screen using a
            // random duration removing the star when it passes the
            // bottom edge.
            //
            ...
            // TODO: run action sequence to create parallax animation effect
